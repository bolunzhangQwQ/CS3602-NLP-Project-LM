...
def from_example_list(args, ex_list, device='cpu', train=True):
    ...
    if train:
        ...
        batch.skip_len = [ex.skip_len for ex in ex_list]
        #sentence_mask = [[0] * ex.skip_len + [1] * (len(ex.tag_id) - ex.skip_len) + [0] * (max_tag_lens - len(ex.tag_id)) for ex in ex_list]
        ...
        #batch.sentence_mask = torch.tensor(sentence_mask, dtype=torch.float, device=device)
    else:
        batch.labels = None
        batch.tag_ids = None
        batch.tag_mask = None
        batch.sentence_mask = None
