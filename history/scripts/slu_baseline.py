...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            best_result['train_loss'], best_result['train_acc'], best_result['train_f1'] = train_loss, train_acc, train_fscore
            torch.save({
            ...
            print('\tTrain loss: %.4f\tTrain acc: %.2f\tTrain fscore(p/r/f): (%.2f/%.2f/%.2f)' % (train_loss, train_acc, train_fscore['precision'], train_fscore['recall'], train_fscore['fscore']))

    print('FINAL BEST RESULT: \tEpoch: %d\tDev loss: %.4f\tDev acc: %.4f\tDev fscore(p/r/f): (%.4f/%.4f/%.4f)' % (best_result['iter'], best_result['dev_loss'], best_result['dev_acc'], best_result['dev_f1']['precision'], best_result['dev_f1']['recall'], best_result['dev_f1']['fscore']))
    print('\tTrain loss: %.4f\tTrain acc: %.4f\tTrain fscore(p/r/f): (%.4f/%.4f/%.4f)' % (best_result['train_loss'], best_result['train_acc'], best_result['train_f1']['precision'], best_result['train_f1']['recall'], best_result['train_f1']['fscore']))
else:
    start_time = time.time()
    metrics, dev_loss = decode('dev')
    dev_acc, dev_fscore = metrics['acc'], metrics['fscore']
